...
RSpec.describe EventPolicy do 
  ...
  let(:event_with_password) { FactoryBot.create(:event, user: user, pincode: '777777') }
  ...
  let(:user_context_with_correct_cookies) { UserContext.new(second_user,
    { "events_#{event_with_password.id}_pincode" => '777777' }, '') }

  let(:user_context_with_incorrect_cookies) { UserContext.new(second_user,
